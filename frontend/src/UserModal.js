...
const UserModal = ({ showModal, modalOpen, modalClose, user, setUser, setUpdateUserId, setShowPass, loadingUser, setLoadingUser, sizeClass, setSizeClass }) => {
  ...
  const updateUserId = value => {
    ...
    if(value=='') {
      ...
      fetch('/update_login_id?user_id='+user.user_id+'&new_login_id='+value)
        .then((res) => res.json())
        .then(
          (data) => {
            if(data.result===1) {
  ...
  const updateUserName = value => {
    if(value=='') {
      ...
      fetch('/update_user_name?user_id='+user.user_id+'&user_name='+value)
        .then((res) => res.json())
        .then(
          (data) => {
            if(data.result===1) {
  ...
  const onChangeAdmin = e => {
    ...
    fetch('/update_admin?user_id='+user.user_id+'&admin='+e.target.value)
      .then((res) => res.json())
      .then(
        (data) => {
          if(data.result===1) {
  ...
  const onChangeSize = e => {
    ...
    fetch('/update_size?user_id='+user.user_id+'&size='+e.target.value)
      .then((res) => res.json())
      .then(
        (data) => {
          if(data.result===1) {
