...
function Todo({ array }) {
  ...
  const rowUpdate = () => {
    ...
    fetch('/update_hed_name?no='+datas.no+'&name='+datas.name)
      .then((res) => res.json())
      .then(
        (data) => {
          /* 更新失敗時 */
  ...
  /* 並び順変更実行 */
  const onClickSort = () => {
    history.push({ pathname: '/HedSort', state: { data: items, title: 'ToDo List' }});
  }
  ...
  const handleAdd = text => {
    ...
    fetch('/insert_hed?no='+insert_no+'&name='+text+'&sort='+insert_sort)
      .then((res) => res.json())
      .then(
        (data) => {
          if(data.result===1) {
  ...
  const dtlAdd = () => {
    ...
    fetch('/insert_dtl?no='+no+'&name='+name)
      .then((res) => res.json())
      .then(
        (data) => {
          if(data.result===1) {
            dtlAddData(no, data.dtl_no, name, data.sort);
          }else {
            alert('insert failed');
            /* ローディング非表示 */
            setLoading(false);
  ...
  /* 追加処理(画面) */
  const dtlAddData = (no, dtl_no, text, sort) => {
  ...
  const dtlUpd = () => {
    ...
    fetch('/update_dtl_name?no='+no+'&dtl_no='+dtl_no+'&name='+name)
      .then((res) => res.json())
      .then(
        (data) => {
          if(data.result===1) {
  ...
  const dtlDel = () => {
    ...
    fetch('/delete_dtl?no='+no+'&dtl_no='+dtl_no)
      .then((res) => res.json())
      .then(
        (data) => {
          if(data.result===1) {
      ...
      <div>
        ...
        <div class="panel">
          ...
          <div class="panel-block" style={style}>
            <span class="column is-10" style={style2}>
              全 {items.length} 件
            </span>
...
/* リスト中身 */
function TodoItem({ items, setItems, item, onCheck, onClickDel, onClickHedUpd, onLoading, onClickDtlAdd, onClickDtlUpd, onClickDtlDel }) {
        ...
        (data) => {
          if(data.result===1) {
            ...
            /* ソート */
              ...
              history.push({ pathname: '/DtlSort', state: { data: data.data, title: item.name, no: item.no }});
            }
          /* 検索件数0件 */
          }else if(data.result===2) {
            /* 詳細追加時の検索済み/未検索の判別用 */
  ...
  const dtlSort = () => {
    ...
    /* １件のみの場合 */
      ...
      history.push({ pathname: '/DtlSort', state: { data: dtlItems, title: item.name, no: item.no, items: items }});
    }
