...
exports.insert_user = function(req, res) {
  ...
  async function exec_insert_user(params, user_id) {
    ...
    const sql2 = 'INSERT INTO LIST_DATA(USER_ID,LIST_NO,NAME,COLOR)VALUES($1,$2,$3,$4)';
    try {
      await dbm.begin();
      const result = await dbm.execute(sql, params);
      await dbm.execute(sql2, [user_id, 1, '優先度高リスト', 'ff0000']);
      await dbm.execute(sql2, [user_id, 2, '保留', 'ffff00']);
      await dbm.execute(sql2, [user_id, 3, '新規リスト', '00ffff']);
      cnt = result.rowCount;
      await dbm.commit();
    } catch (e) {
      await dbm.rollback();
...
exports.update_user = function(req, res) {
  ...
  async function select_mst(params) {
    ...
    var result;
    const dbm = await getPostgresClient();
    const sql = 'SELECT COUNT(*) AS CNT FROM USER_MST WHERE LOGIN_ID = $1 AND USER_ID <> $2';
    try {
      result = await dbm.execute(sql, params);
      cnt = result.rowCount;
    } catch (e) {
  ...
  async function exec_update_user(params, user_id) {
    ...
    var cnt = 0;
    const dbm = await getPostgresClient();
    const sql = 'UPDATE USER_MST SET LOGIN_ID = $1, USER_NAME = $2, PASS = $3, ADMIN = $4, FONT_SIZE = cast($5 as integer) WHERE USER_ID = $6';
    try {
      await dbm.begin();
      const result = await dbm.execute(sql, params);
      cnt = result.rowCount;
...
/* パスワード検索 */
exports.select_pass = function(req, res) {
  res.header("Content-Type", "application/json; charset=utf-8");
  var apiName = '[select_pass]';
  console.log(apiName+': start');
  ...
  async function select_pass(params) {
    ...
    var result;
    const dbm = await getPostgresClient();
    const sql = 'SELECT PASS FROM USER_MST WHERE USER_ID = $1';
    try {
      result = await dbm.execute(sql, params);
      cnt = result.rowCount;
    } catch (e) {
